...
//        	return '<span title="'+defaultValue+'">'+defaultValue.substr(0,50)+'...</span>'
//        }
        return defaultValue;
    };
...
//        clickToSelect: false,
        ...
        searchTimeOut: 500,
        longTextCut:20,//文本过长时剪切，鼠标悬浮时提示
...
//            return 'No matching records found';
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            html.push(sprintf('<div class="keep-open btn-group" title="%s">',
                ...
                '<button type="button" style="border:0px;width:50px; height:30px;border-radius: 5px; background:#6FAED9; line-height:30px; text-align:center; color:#fff;font-size:13px;" data-toggle="dropdown">',
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        var pageNumber = [
            ...
            '<button type="button" style="border:0px;width:50px; height:30px;border-radius: 5px; background:#6FAED9; line-height:30px; text-align:center; color:black;font-size:13px;" data-toggle="dropdown">',
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                style = sprintf('style="word-break:break-all;%s"', csses.concat(that.header.styles[j]).join('; '));

//                value = calculateObjectValue(that.header,
//                    that.header.formatters[j], [value, item, i], value);
                //当文本过长时剪切，鼠标悬浮时提示
                value = that.calculateValue(that.header, that.header.formatters[j], [value, item, i], value);
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = ['<td class="bs-checkbox" style="text-align:center; vertical-align:middle">',
                        '<input' +
                            sprintf(' data-index="%s"', i) +
    ...
    BootstrapTable.prototype.initServer = function (silent, queryparams) {
        ...
        var that = this,
            ...
            params = {
                ...
                sealmark:$("#sealmark").val()
            };
        if (!this.options.url) {
            return;
        }

      //设置往服务器端传送的参数，queryParamsType为limit的时候是rest方式传值
        if (this.options.queryParamsType === 'limit') {
            params = {
            	//设置往服务器端传送的一个页面显示多少条数据
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (that.options.sidePagination === 'server') {
                    that.options.totalRows = res.totalCount;
                    //设置返回数据是第几页的数据
                    that.options.pageNumber = res.pageNo;
                  //设置返回数据是一页显示多少条数据
                    that.options.pageSize = res.pageSize;
                    data = res.rows;
    ...
    BootstrapTable.prototype.updateSelected = function () {
        ...
        */
        //20150528 zhangxinm 修改bug：bootstraptable初始化后默认选中的项目无法通过getSelected获得
        var that = this;
        this.$selectItem.each(function () {
        	var checked = $(this).prop('checked');
            $(this).parents('tr')[checked ? 'addClass' : 'removeClass']('selected');
            var index = $(this).parents('tr').attr("data-index");
            that.data[index][that.header.stateField] = checked;
        });
    };
    ...
    BootstrapTable.prototype.getDeselections = function () {
    	var that = this;
    ...
    BootstrapTable.prototype.getPageSize = function(){
    	return this.options.pageSize;
    }
    
    BootstrapTable.prototype.getPageNo = function(){
    	return this.options.pageNo;
    }
    
    BootstrapTable.prototype.getSort = function(){
    	return this.options.sort;
    }
    
    BootstrapTable.prototype.getOrder = function(){
    	return this.options.order;
    }
    
    //包含上方line80的calculateObjectValue方法内容，并且在文本过长时剪切，鼠标悬浮时提示完整信息
    BootstrapTable.prototype.calculateValue = function(self, name, args, defaultValue){
    	 if (typeof name === 'string') {
             // support obj.func1.func2
             var names = name.split('.');
         ...
         if (typeof name === 'function') {
        	var name = name.apply(self, args);
        	if( typeof name === 'string' && name.indexOf('a') > 0 ){
        		 name = name.split(' ');
            	 if(name[0]=='<a'){
            		 name[0] += ' title=' +'\''+ escapeHTML(defaultValue) +'\''+' ';
         ...
         /* if(typeof defaultValue === 'string' && defaultValue.length > this.options.longTextCut){
         	return '<span title="'+escapeHTML(defaultValue)+'">'+defaultValue.substr(0, this.options.longTextCut)+'...</span>'
         }*/
         if(typeof defaultValue === 'string'){
          	return '<span title="'+escapeHTML(defaultValue)+'">'+defaultValue+'</span>'
          }
         return defaultValue;
    }
    ...
    var allowedMethods = [
        'getSelections', 'getData', 'getDeselections',
        'load', 'append', 'remove',
        'updateRow',
        'mergeCells',
        'checkAll', 'uncheckAll',
