...
$(function (){
	...
	$("#save").click(function (){
		...
		if(inputId == "manager" || inputId == "charge" || inputId == "project_manager"){
			if(stringJson.indexOf("},{") >= 0){
				if(inputId == "manager"){
					layer.msg("您只能选择一个部门管理人");
				}else{
	...
	function search(){
		...
		var path="${_baseUrl}/userController/selectDepartUser?set_document_departId=${set_document_departId}";
	 	 $.ajax({    
			type: "post",    
			async: false,    
			url:path,   
			    ...
			    if(obj=="[]"){
					...
					for(var i=0;i<data.length;i++){	
						...
						if(managerIdSplit.indexOf(data[i].id) >= 0){
							innerHtml+="<p><input checked='checked' name='"+data[i].id+"_chk' id='"+data[i].id+"' value='"+data[i].userName+"' type='checkBox' onClick=\"addSelectUser('"+data[i].userId+"','"+data[i].userName+"','"+data[i].id+"');\"><span>"+data[i].userName+"</span></p>";
							//判断如果右侧已存在，则不需要在加入
							var flag = true;
							...
							if("${docManager}" !=data[i].id){
								innerHtml+="<p><input name='"+data[i].departId+"_chk' id='"+data[i].id+"' value='"+data[i].userName+"' type='checkBox' onClick=\"addSelectUser('"+data[i].userId+"','"+data[i].userName+"','"+data[i].id+"');\"><span>"+data[i].userName+"</span></p>";
							}
						}
					}
				}                 
			...
			success: function (data) {   
			    ...
			    if(obj=="[]"){
					...
					for(var i=0;i<data.length;i++){	
						...
						if(managerIdSplit.indexOf(data[i].id) >= 0){
							innerHtml+="<p><input checked='checked' name='"+treeNode.id+"_chk' id='"+data[i].id+"' value='"+data[i].userName+"' type='checkBox' onClick=\"addSelectUser('"+data[i].userId+"','"+data[i].userName+"','"+data[i].id+"');\"><span>"+data[i].userName+"</span></p>";
							//判断如果右侧已存在，则不需要在加入
							var flag = true;
							for(var ii=0;ii<selectUserList.length;ii++){
					 			var obj = JSON.parse(selectUserList[ii]);
							...
							if("${docManager}" !=data[i].id){
								innerHtml+="<p><input name='"+treeNode.id+"_chk' id='"+data[i].id+"' value='"+data[i].userName+"' type='checkBox' onClick=\"addSelectUser('"+data[i].userId+"','"+data[i].userName+"','"+data[i].id+"');\"><span>"+data[i].userName+"</span></p>";
			 				}
						}
					}
				}                 
