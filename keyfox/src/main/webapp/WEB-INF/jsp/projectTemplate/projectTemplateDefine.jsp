	...
	var options = {
			...
			success:function(json){
				layer.msg(json.message,{shift:5,time:1500},function(){
					parent.closeWin();
					parent.refreshTable();
					parent.addDemandTrace(json.data); 
...
function judgeName(){
	var path="${_baseUrl}/projectTemplateTypeController/judgeProTemplateName";
	var name = $("#projectTemplateName").val();
	if(name !=""){
		$.ajax({    
			type: "POST",    
			async: false,    
...
$(function(){
	$("#selectByTemplateName").keydown(function(event) {//给输入框绑定按键事件
...
<body>
<form action="${_baseUrl}/projectTemplateTypeController/addProjectTemplate" method="post" name="form" id="form">  
 <input type="hidden" name="documentId" id="documentId" value="" />
 <div class="popup_tit mtmb20">新增项目模板</div>
 <div class="jbinformation roleshux">
	<table width="743" border="0" cellspacing="0" cellpadding="0">
  <tr>
  	 <td width="138" height="50" align="center" bgcolor="#f7f4f4">模板名称</td>
    <td width="246" valign="middle"><div class="jbxinxi_s" style=" width: 210px"><input type="text" name="projectTemplateName"  style="width:200px; height:30px;" id="projectTemplateName" onblur="judgeName();" class="jbxinxi_input validate[maxSize[50]]"></div><div class="jbxinxi_span1">*</div></td>    <!-- 提示信息重复，取其一  -->
        ...
        <option>请选择</option>
         <c:forEach items="${listStyleTemplateDefine }" var="styleSetTemplate">
			<c:if test="${styleSetTemplate.styleType=='inSet'}">
				<option title="${styleSetTemplate.styleDocumentName}" value="${styleSetTemplate.id}">${styleSetTemplate.styleDocumentName} </option>
			</c:if>
		</c:forEach>
    ...
    <select class="qr_select qr_select2  validate[required,funcCall[checkSelectOut]]"   name="styleOutTemplate" style="width:210px; padding:5px 0;">
          ...
          <c:forEach items="${listStyleTemplateDefine }" var="styleOutTemplate">
			<c:if test="${styleOutTemplate.styleType=='outSet'}">
				<option  title="${styleOutTemplate.styleDocumentName}" value="${styleOutTemplate.id}">${styleOutTemplate.styleDocumentName}</option>
			</c:if>
		</c:forEach>
      </select></div><div class="jbxinxi_span1">*</div></td>
  </tr>
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" class="tablebox">
        <thead>
         ...
         <c:forEach items="${listDocumentTemplate }" var="documentTemplate" varStatus="status">
			<tr>
			<td><input name="document" value="${documentTemplate.id}" onclick="judgeSelectDoc(this);"   type="checkbox"  /></td>
			<td>${status.count}</td>
...
<div class="permission_an mubanclass_an ma mt30">
	<shiro:hasPermission name="projectTemplateTypeController:addProjectTemplate">
