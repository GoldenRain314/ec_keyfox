...
<div class="chapter_popup" style="width:276px;">
  <div class="popup_tit mtmb20">添加附录</div>
    ...
    <form action="${_baseUrl}/documentSectionController/saveChangeSection" id ="form" name ="form" method="post">
    	...
    	<input type="hidden" value="${sectionNumber }"  id="sectionNumber" name="sectionNumber"/>
            <select class="input_text validate[required]" style="width: 276px;" id="sectionName"  name="sectionName" >
            	<option value="">请选择</option>
            	<c:forEach items="${listAdd}" var="appendix">
            		<option value ="${appendix.sectionName}">${appendix.sectionName}</option>
            	</c:forEach>
            </select>
   	</form>
    </div>
