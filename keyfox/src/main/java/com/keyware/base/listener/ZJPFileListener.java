...
import com.keyware.base.utils.FileUtils;
...
public class ZJPFileListener implements FileAlterationListener {
    ...
    String filePath;
	public ZJPFileListener(String dbName,String collTableName,String collTotalName,String filePath){
		this.dbName = dbName;
		this.collTableName = collTableName;
		this.collTotalName = collTotalName;
    ...
    public void onStart(FileAlterationObserver fileAlterationObserver) {
        ...
        MongoCollection<Document> coll = MongoDBUtils.instance.createColl(dbName, collTableName);

        File files[] = FileUtils.getFileName(filePath);
        if (files == null) {
            return;
        }
        for (File file : files) {
...
//        importData(coll, document);
    ...
    private void foreachTable() {
        MongoCollection<Document> coll = MongoDBUtils.instance.getCollection(dbName, collTableName);
    ...
    private void importData(MongoCollection<Document> collTable, Document doc) {
        ...
        MongoCollection<DBObject> coll = MongoDBUtils.instance.createCollObjeck(dbName, collTotalName);

        System.out.println(
            "=========================+==================正在导入  " + doc.getString("tableName")
                + "   数据");

        File file = new File(filePath + doc.getString("tableName"));
        boolean flag = true;
        while (flag) {
